  ...
  MACHINES.each do |boxname, boxconfig|
      ...
      config.vm.define boxname do |box|
          ...
          box.vm.provision "shell", inline: <<-SHELL
			...
			yum install -y httpd
			cp /vagrant/script/alert.sh /opt
			...
			yum install epel-release -y && yum -y install spawn-fcgi php php-cli mod_fcgid httpd
			sudo rm -f /etc/sysconfig/spawn-fcgi
