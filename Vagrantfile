...
Vagrant.configure("2") do |config|
    ...
    config.vm.provision "shell", inline: <<-SHELL
        ...
        echo "192.168.56.101 backupserver" >> /etc/hosts
        echo "192.168.56.102 client" >> /etc/hosts
        echo "192.168.56.111 ansiblecontroller" >> /etc/hosts
        SHELL
    ...
    config.vm.define "backupserver", primary: true do |backupserver|
            ...
            backupserver.vm.network "private_network", ip: "192.168.56.101", virtualbox__intnet: true, name: "provision_net"
        end

    config.vm.define "client", primary: true do |client|
            client.vm.hostname = "client"
            client.vm.network "private_network", ip: "192.168.56.102", virtualbox__intnet: true, name: "provision_net"
        end

    config.vm.define "ansiblecontroller", primary: true do |ansiblecontroller|
            ansiblecontroller.vm.network "private_network", ip: "192.168.56.111", virtualbox__intnet: true, name: "provision_net"
